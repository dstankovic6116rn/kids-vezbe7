...
public class MultipleServentStarter {
	...
	private static void startServentTest(String testName) {
		...
		for(int i = 0; i < serventCount; i++) {
			try {
				ProcessBuilder builder = new ProcessBuilder("java", "-cp", "out\\production\\KiDS-Vezbe7", "app.ServentMain",
		...
		PriorityQueue<Test2> queue = new PriorityQueue<>();
		Iterator<Map.Entry<String, String>> ite =  hm.entrySet().iterator();
		SortedMap<Test2, Test2> test = new TreeMap<>();
		Deque<String> dq = new ArrayDeque<>();
		Map<String, String> mpp = new TreeMap<>();
